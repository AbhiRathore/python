...
model_xgb <- xgboost(data=as.matrix(train[, c(2:60)]),label=as.matrix(X_target), 
                      objective="binary:logistic", nrounds=1000, eta=0.005, max_depth=10, subsample=0.75, 
...
plot(pred)
pred2=factor(ifelse(pred>.96,1,0))

confusionMatrix(data = pred,reference = val$is_promoted,positive = '1')



####### test data

pred2 <- predict(model_xgb, as.matrix(test[, c(2:60)]))
test$is_promoted=pred2

sub1=data.frame(test$employee_id,test$is_promoted)
head(sub)
